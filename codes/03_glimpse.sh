...
POOL="${WORKDIR}/_duo_pooled"
GLIMPSE="<where glimpse is installed>/glimpse2-src"
mkdir -p "${POOL}"
...
python "${DUO}/src/Duopogen.py" germline-glimpse  \
  ...
  -s all --map-dir "${GLIMPSE}/maps/genetic_maps.b38" \
  --panel-dir "${RES}/glimpse_panel_noNA12878" \
  -t 14 --phase-threads 2 
